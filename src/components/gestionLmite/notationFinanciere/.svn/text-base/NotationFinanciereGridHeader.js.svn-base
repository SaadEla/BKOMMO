...
import {loadingRemoveSelector} from "../../../redux/GestionDeLimite/notationFinanciere/Selectors";


export function NotationFinanciereGridHeader({notationFinanciereRefloading,ajouterNotation,ajouter, supprimer, quickSearch, quickSearch_changeHandler, actualiser, selectedRow}) {
    ...
    return <Row style={{width: '100%'}}>
        <Col span={18}>
            <div>
                <Button onClick={ajouterNotation} icon="filter-add-expression" style={{borderRadius: '10px', color: 'rgba(19,105,180,0.9)'}}
                        primary={true}
                        look="bare">Notation financière</Button>

                    <Button onClick={ajouter} icon="wrench" style={{borderRadius: '10px', color: 'rgba(19,105,180,0.9)'}}
                            primary={true}
                            look="bare">
                        <Spin spinning={notationFinanciereRefloading}>Paramètres</Spin></Button>
