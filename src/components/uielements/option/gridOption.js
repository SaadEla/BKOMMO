...
import {selectingGridRow} from '../../../redux/dealCapture/option/optionSlice';
import {
    mappingListOption, natureOptionLabelFunction
} from "./OptionUtil";
import {ExcelExport} from '@progress/kendo-react-excel-export';
import {Spin} from "antd";
import {dealOptionDetailLoadingSelector, OptionListLoadingSelector} from "../../../redux/dealCapture/selectors/OptionSelectors";
...
const DataGrid = ({selectedRow,heightGrid = '700px', setExportCsv, data, children, handleRowDoubleClick}) => {
    ...
    const optionLoading = useSelector(OptionListLoadingSelector);
    const dealOptionLoading = useSelector(dealOptionDetailLoadingSelector);
    ...
    useEffect(function () {
        setDataGrid(mappingListOption(data))
    }, [data])
    ...
    return (
        <Spin spinning={optionLoading || dealOptionLoading} size="large">
            <ExcelExport
                data={dataGrid}
                ref={setExportCsv}
                fileName={"option " + moment().format("DD-MM-YYYY HH_mm_ss")}
            >
                <Grid
                ...
                >
                    ...
                    <Column format="{0:dd/MM/yyyy}" field="valueDateLabel" title="Value Date"/>
                    ...
                    <Column className={"centerAlign"}  field="typeOptionLabel" title="Type option"/>
                    <Column className="centerRight" format="{0:n2}" field="strike" title="Strike"/>
                    <Column  width={'90px'} className="centerRight" format="{0:n2}" field="quantite" title="Quantité"/>
                    <Column  width={'80px'} className="centerRight" format="{0:n2}" field="prixOptionLabel" title="Prix option"/>
