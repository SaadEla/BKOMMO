...
import {optionsContrePartie, tiersLabelFunction2} from "./OptionUtil";
import {selectionSettlementOptionAPI} from "../../../redux/dealCapture/rest/OptionRestCall";
import {
    loadingSelectSettlementOptionSelector,
    selectionSettlementOptionSelector
} from "../../../redux/dealCapture/selectors/OptionSelectors";

const {Option} = Select;

export function AvisSettlementOption({references}) {
    const dispatch = useDispatch();
    const [settlementDataGrid, setSettlementDataGrid] = useState([])
    const selectionSettlement = useSelector(selectionSettlementOptionSelector);
    const loadingSelectSettlementOption = useSelector(loadingSelectSettlementOptionSelector);
    ...
    function onContrePartiechange(value) {
        ...
        dispatch(selectionSettlementOptionAPI({contrepartieId: value}))
    }
    ...
    return <>
        ...
        <Spin spinning={loadingSelectSettlementOption} size="large">
